...
int main(int argc, char * argv[])
{
  ...
  std::shared_ptr<rclcpp::Node> node = rclcpp::Node::make_shared("service_node1");

  //for client
  if (argc == 4) { // change argc == 3 if you are running with CLI
    ...
    RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "Node1 is running as a client...");
  ...
  // for service
  else if (argc == 2){ // change argc == 1 if you are running with CLI
    ...
    RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "Node1 is running as a server...");
    RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "Ready to multiply two ints.");
    rclcpp::spin(node);
  }

  else {
    RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "\nusage: \nsrv_node1 X Y (for client)\nsrv_node1 (for service)");
    return 1;
  }
